...
iptables -A OUTPUT -o eth0 -m state --state NEW -m set --match-set white_list src -j ACCEPT
iptables -A OUTPUT -o eth0 -m state --state NEW -m set --match-set iran_ipv4 src -j REJECT_WITH
